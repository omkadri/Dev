...
public class Ball : MonoBehaviour
{
    ...
    [Tooltip("How long the preview line should be before launch.")]
    [SerializeField] float _previewLineLength = 2f;
    ...
    void Update()
    {
        // Draw preview line before launch
        if (!_isActivated)
        {
            Vector3 mousePos = _mainCamera.ScreenToWorldPoint(Mouse.current.position.ReadValue());
            mousePos.z = 0f;
            Vector2 previewDir = (mousePos - transform.position).normalized;

            // Clamp the preview direction as well
            previewDir = ClampLaunchDirection(previewDir);

            Debug.DrawLine(transform.position,
                transform.position + (Vector3)(previewDir * _previewLineLength),
                Color.green);
        }
    }

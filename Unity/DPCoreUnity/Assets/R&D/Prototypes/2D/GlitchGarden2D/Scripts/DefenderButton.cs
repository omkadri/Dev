...
public class DefenderButton : MonoBehaviour
{
    ...
    {
        var buttons = FindObjectsByType<DefenderButton>( FindObjectsSortMode.None ); // finds all objects with DefenderButton script attached to them
        foreach(DefenderButton button in buttons)
        {
            button.GetComponent<SpriteRenderer>().color = new Color32(41, 41, 41, 255); //greys out all DefenderButtons
            FindFirstObjectByType<DefenderSpawner>().SetSelectedDefender(defenderPrefab); //passes currently clicks defender into DefenderSpawner
        }
